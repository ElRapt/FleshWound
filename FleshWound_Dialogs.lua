...
--- Creates a generic dialog frame.
-- @param name string Global name of the dialog.
-- @param titleText string The dialog title.
...
--- Creates a multi-line edit box with a character counter.
-- @param parent Frame The parent frame.
...
--- Creates a single-line edit box with a character counter.
-- @param parent Frame The parent frame.
-- @param maxChars number Maximum characters.
...
--- Opens the wound dialog for a region.
-- @param regionID number The body region ID.
-- @param skipCloseDialogs boolean Optional flag to skip closing dialogs.
...
--- Creates a visual entry for a single note.
-- @param parent Frame The parent frame.
-- @param note table The note data.
-- @param index number The note index.
-- @param regionID number The body region ID.
-- @return Frame The created note entry.
function Dialogs:CreateNoteEntry(parent, note, index, regionID)
    ...
    local sev = GUI.SeveritiesByID[severityID] or { color = {0,0,0,0} }
...
--- Opens the note dialog for adding or editing a note.
-- @param regionID number The body region ID.
...
--- Populates the note dialog with existing data if editing.
-- @param dialog Frame The note dialog.
-- @param noteIndex number|nil The note index if editing.
function Dialogs:PopulateNoteDialog(dialog, noteIndex)
    ...
    local function ConfirmAndSaveNote()
        ...
        if isEdit and notes and notes[noteIndex] then
            ...
            table.insert(woundData[dialog.regionID], { text = text, severityID = severityID, statusIDs = chosenStatuses })
        end
        GUI:UpdateRegionColors()
        dialog.EditBox:SetText("")
        dialog:Hide()
...
--- Closes dialogs based on the specified type.
